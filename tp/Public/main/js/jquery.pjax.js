...
(function($) {
 ...
 // Ignore anchors on the same page
		if (link.hash
				&& link.href.replace(link.hash, '') === location.href.replace(
						location.hash, ''))
			return
 ...
 // If popping back to the same state, just skip.
			...
			if (container.length) {
				...
				if (pjax.state) {
					// Since state ids always increase, we can deduce the
					// history
				...
				if (contents) {
					...
					container
							.trigger(beforeReplaceEvent, [ contents, options ])
	...
	function stripPjaxParam(url) {
		return url.replace(/\?_pjax=[^&]+&?/, '?').replace(/_pjax=[^&]+&?/, '')
				.replace(/[\?&]$/, '')
	}
	...
	function extractContainer(data, xhr, options) {
		...
		obj.url = stripPjaxParam(xhr.getResponseHeader('X-PJAX-URL')
				|| options.requestUrl)
